  ...
  <!-- Horizontal Form -->
       ...
       <form method="post" action="{{ url()->current() }}">
          ...
          <div class="box-body">
            <div class="form-group field_wrapper">
                ...
                <div>
                    ...
                    @foreach($apa as $transaksiToko)
                    <option value="{{ $transaksiToko->id }}">{{ $transaksiToko->nama_barang}}</option>
                    @endforeach
                    </select>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
          <?php $count = 1; ?>@foreach($a as $transaksiToko)
            <tr>
                <td>{{ $transaksiToko->created_at}}</td>
                <td>{{ $transaksiToko->nama_barang}}</td>
                <td>{{ $transaksiToko->satuan }}</td>
                <td>{{ $transaksiToko->jumlah }}</td>
                <td>Rp. {{ number_format($transaksiToko->harga)}}</td>

                <td>
                    <a href="item/{{$transaksiToko->id}}" class="btn btn-success">Edit</a>
                    <a href="{{$transaksiToko->idtrans}}/delete" onclick="return confirm ('Apakah Anda Yakin Menghapus Data Ini ? ');" class="btn btn-danger">Hapus</a>
                    </td>
